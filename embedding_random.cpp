...
vector<vector<int>> initializeVerticesToQubitsMapping(int size)
{
...
vector<int> initializeQubitsToVerticesMapping(int size)
{
    vector<int> mapping;
    for (int i = 0; i < size; i++)
    {
...
void dijkstraComputePaths(int source, Graph &adjacency_list,
                          vector<int> &min_distance, vector<int> &previous, vector<int> qubitToVertexMapping, int currentVertex)
{
    ...
    min_distance[source] = QUBIT_WEIGHT;
    ...
    while (!vertex_queue.empty())
    {
        ...
        if (dist > min_distance[currentVertex]) // Find the shortest path from root to current vertex.
        {
            return;
        }
        ...
        for (int i = 0; i < adjacency_list.deg(u); i++)
        {
            ...
            if (qubitToVertexMapping.at(v) == -1)
            {
                continue;
            }
            int weight = QUBIT_WEIGHT;
...
void createDummyGraph(Graph &dummyGraph, Graph &G, Graph &H, vector<vector<int>> mapping, int currentVertex, int index)
{
    dummyGraph = G;
...
void updateModels(Graph &G, Graph &H, int root, int currentVertex, vector<int> &qubitToVertexMapping, vector<vector<int>> &vertexToQubitMapping, float ratio)
{
    ...
    for (int i = 0; i < H.deg(currentVertex); i++)
    {
        ...
        if (vertexToQubitMapping.at(H.adj[currentVertex].at(i)).size() != 0)
        {
            ...
            vector<int> previous;
            dijkstraComputePaths(root, dummyGraph, min_distance, previous, qubitToVertexMapping, currentVertex);
    ...
    for (map<int, int>::iterator ii = vertex_usage.begin(); ii != vertex_usage.end(); ++ii)
    {
        ...
        { // appears in more than one path, assign to new model ****************************
            if (find(vertexToQubitMapping.at(currentVertex).begin(), vertexToQubitMapping.at(currentVertex).end(), (*ii).first) == vertexToQubitMapping.at(currentVertex).end())
            {
                vertexToQubitMapping.at(currentVertex).push_back((*ii).first);
                qubitToVertexMapping.at((*ii).first) = currentVertex;
                extendChain(G, H, vertexToQubitMapping, qubitToVertexMapping, currentVertex, (*ii).first, ratio);
            }
        }
        else
        { // appears once, update
...
void findMinimalVertexModel(Graph &G, Graph &H, vector<vector<int>> &vertexToQubitMapping, vector<int> &qubitToVertexMapping, int currentVertex, float ratio)
{
    ...
    for (int i = 0; i < H.deg(currentVertex); i++)
    {
        for (int j = 0; j < vertexToQubitMapping.at(H.adj[currentVertex].at(i)).size(); j++)
        {
            for (int k = 0; k < G.adj[vertexToQubitMapping.at(H.adj[currentVertex].at(i)).at(j)].size(); k++)
            {
                int neigh = G.adj[vertexToQubitMapping.at(H.adj[currentVertex].at(i)).at(j)].at(k);
                if (find(rootCandidate.begin(), rootCandidate.end(), neigh) == rootCandidate.end() && qubitToVertexMapping.at(neigh) == -1)
                { // if not contain
    ...
    for (int i = 0; i < rootCandidate.size(); i++)
    {
        ...
        for (int j = 0; j < H.deg(currentVertex); j++)
        {
            ...
            else
            {
                ...
                Graph dummyGraph(0);
                createDummyGraph(dummyGraph, G, H, vertexToQubitMapping, currentVertex, j);
                vector<int> min_distance;
                vector<int> previous;
                dijkstraComputePaths(current, dummyGraph, min_distance, previous, qubitToVertexMapping, currentVertex);
    ...
    //find the union paths from root g* to each vertex model
    updateModels(G, H, root, currentVertex, qubitToVertexMapping, vertexToQubitMapping, ratio);
}
